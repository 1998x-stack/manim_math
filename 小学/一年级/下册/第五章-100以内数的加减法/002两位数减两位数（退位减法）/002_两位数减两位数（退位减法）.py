...
class TwoDigitSubtractionBorrow(Scene):
    ...
    def scene_2_introduce_problem(self):
        ...
        # -- 52 的结构 --
        label_52 = Text("52", font="Noto Sans CJK SC", font_size=60, color=self.COLOR_TEXT).move_to(UP * 4.2 + LEFT * 2.5)
        eq1 = Text("=", font="Noto Sans CJK SC", font_size=50, color=self.COLOR_SUB).move_to(UP * 4.2 + LEFT * 0.9)
        tens_52 = Text("5个十", font="Noto Sans CJK SC", font_size=32, color=self.COLOR_TENS).move_to(UP * 4.2 + RIGHT * 0.6)
        plus1 = Text("+", font="Noto Sans CJK SC", font_size=40, color=self.COLOR_SUB).move_to(UP * 4.2 + RIGHT * 2.1)
        ones_52 = Text("2个一", font="Noto Sans CJK SC", font_size=32, color=self.COLOR_ONES).move_to(UP * 4.2 + RIGHT * 3.4)

        self.play(Write(label_52), run_time=0.4)
        self.play(FadeIn(eq1), FadeIn(tens_52), FadeIn(plus1), FadeIn(ones_52), run_time=0.6)

        # -- 27 的结构 --
        label_27 = Text("27", font="Noto Sans CJK SC", font_size=60, color=self.COLOR_TEXT).move_to(UP * 2.5 + LEFT * 2.5)
        eq2 = Text("=", font="Noto Sans CJK SC", font_size=50, color=self.COLOR_SUB).move_to(UP * 2.5 + LEFT * 0.9)
        tens_27 = Text("2个十", font="Noto Sans CJK SC", font_size=32, color=self.COLOR_TENS).move_to(UP * 2.5 + RIGHT * 0.6)
        plus2 = Text("+", font="Noto Sans CJK SC", font_size=40, color=self.COLOR_SUB).move_to(UP * 2.5 + RIGHT * 2.1)
        ones_27 = Text("7个一", font="Noto Sans CJK SC", font_size=32, color=self.COLOR_ONES).move_to(UP * 2.5 + RIGHT * 3.4)
        ...
        prob_txt = Text(
            "个位：2 < 7，不够减！",
            font="Noto Sans CJK SC", font_size=34, color=self.COLOR_BORROW
        ).move_to(UP * 0.8)
    ...
    def scene_3_borrow_concept(self):
        ...
        # 列标签
        tens_lbl = Text("十位", font="Noto Sans CJK SC", font_size=26, color=self.COLOR_TENS).move_to(UP * 5.2 + LEFT * 1.5)
        ones_lbl = Text("个位", font="Noto Sans CJK SC", font_size=26, color=self.COLOR_ONES).move_to(UP * 5.2 + RIGHT * 1.5)
        self.play(FadeIn(tens_lbl), FadeIn(ones_lbl), run_time=0.4)

        # 52 的十位和个位
        t5 = Text("5", font="Noto Sans CJK SC", font_size=80, color=self.COLOR_TENS).move_to(UP * 3.8 + LEFT * 1.5)
        n2 = Text("2", font="Noto Sans CJK SC", font_size=80, color=self.COLOR_ONES).move_to(UP * 3.8 + RIGHT * 1.5)
        self.play(Write(t5), Write(n2), run_time=0.5)

        # 说明：借位
        explain = Text(
            "个位2不够减7，向十位借1",
            font="Noto Sans CJK SC", font_size=30, color=self.COLOR_BORROW
        ).move_to(UP * 2.3)
        ...
        # 5 -> 4，2 -> 12
        new_t4 = Text("4", font="Noto Sans CJK SC", font_size=80, color=self.COLOR_TENS).move_to(UP * 3.8 + LEFT * 1.5)
        new_n12 = Text("12", font="Noto Sans CJK SC", font_size=68, color=self.COLOR_ONES).move_to(UP * 3.8 + RIGHT * 1.5)
        ...
        txt1 = Text(
            "1个十 = 10个一",
            font="Noto Sans CJK SC", font_size=30, color=self.COLOR_TENS
        ).move_to(UP * 1.0)
        ...
        txt2 = Text(
            "2 + 10 = 12 个一",
            font="Noto Sans CJK SC", font_size=30, color=self.COLOR_ONES
        ).move_to(DOWN * 0.3)
    ...
    def scene_4_vertical_calculation(self):
        ...
        # 列标签
        tens_lbl = Text("十位", font="Noto Sans CJK SC", font_size=22, color=self.COLOR_TENS).move_to(
            np.array([col_tens, 6.0, 0])
        )
        ones_lbl = Text("个位", font="Noto Sans CJK SC", font_size=22, color=self.COLOR_ONES).move_to(
            np.array([col_ones, 6.0, 0])
        )

        # 竖式数字
        d5 = Text("5", font="Noto Sans CJK SC", font_size=70, color=self.COLOR_TENS).move_to(np.array([col_tens, row1_y, 0]))
        d2 = Text("2", font="Noto Sans CJK SC", font_size=70, color=self.COLOR_ONES).move_to(np.array([col_ones, row1_y, 0]))
        d2b = Text("2", font="Noto Sans CJK SC", font_size=70, color=self.COLOR_TENS).move_to(np.array([col_tens, row2_y, 0]))
        d7 = Text("7", font="Noto Sans CJK SC", font_size=70, color=self.COLOR_ONES).move_to(np.array([col_ones, row2_y, 0]))
        minus_sign = Text("−", font="Noto Sans CJK SC", font_size=64, color=self.COLOR_TEXT).move_to(
        ...
        step1_txt = Text(
            "第1步：个位 2 < 7，不够减！",
            font="Noto Sans CJK SC", font_size=28, color=self.COLOR_BORROW
        ).move_to(np.array([0, 0.5, 0]))
        ...
        small_4 = Text(
            "4", font="Noto Sans CJK SC", font_size=38, color=self.COLOR_BORROW
        ).move_to(np.array([col_tens + 0.2, row1_y + 0.7, 0]))
        small_12 = Text(
            "12", font="Noto Sans CJK SC", font_size=34, color=self.COLOR_ONES
        ...
        step3_txt = Text(
            "第3步：个位 12 − 7 = 5",
            font="Noto Sans CJK SC", font_size=28, color=self.COLOR_ONES
        ).move_to(np.array([0, 0.5, 0]))
        self.play(ReplacementTransform(step2_txt, step3_txt), run_time=0.4)

        calc_ones = Text(
            "12 − 7 = 5",
            font="Noto Sans CJK SC", font_size=38, color=self.COLOR_ONES
        ).move_to(np.array([0, -0.6, 0]))
        self.play(Write(calc_ones), run_time=0.5)
        self.wait(0.5)

        result_ones = Text(
            "5", font="Noto Sans CJK SC", font_size=70, color=self.COLOR_RESULT
        ...
        step4_txt = Text(
            "第4步：十位 4 − 2 = 2",
            font="Noto Sans CJK SC", font_size=28, color=self.COLOR_TENS
        ).move_to(np.array([0, 0.5, 0]))
        self.play(ReplacementTransform(step3_txt, step4_txt), FadeOut(calc_ones), run_time=0.4)

        remind_txt = Text(
            "十位借出1后剩4个十",
            font="Noto Sans CJK SC", font_size=26, color=self.COLOR_BORROW
        ).move_to(np.array([0, -0.5, 0]))
        self.play(FadeIn(remind_txt), run_time=0.4)

        calc_tens = Text(
            "4 − 2 = 2",
            font="Noto Sans CJK SC", font_size=38, color=self.COLOR_TENS
        ).move_to(np.array([0, -1.4, 0]))
        self.play(Write(calc_tens), run_time=0.5)
        self.wait(0.4)

        result_tens = Text(
            "2", font="Noto Sans CJK SC", font_size=70, color=self.COLOR_RESULT
        ...
        ans_grp = VGroup(
            Text("52 − 27 =", font="Noto Sans CJK SC", font_size=42, color=self.COLOR_TEXT),
            Text("25", font="Noto Sans CJK SC", font_size=58, color=self.COLOR_RESULT)
    ...
    def scene_5_summary(self):
        ...
        for i, (num, desc1, desc2, color) in enumerate(steps_data):
            ...
            num_txt = Text(num, font="Noto Sans CJK SC", font_size=26, color=WHITE).move_to(
                np.array([-3.8, y_pos, 0])
            )
            t1 = Text(desc1, font="Noto Sans CJK SC", font_size=27, color=color).move_to(
                np.array([0.3, y_pos + 0.38, 0])
            )
            t2 = Text(desc2, font="Noto Sans CJK SC", font_size=27, color=self.COLOR_TEXT).move_to(
                np.array([0.3, y_pos - 0.38, 0])
            )
            sep_line = Line(
        ...
        remind_grp = VGroup(
            Text("记住！十位已借出1，", font="Noto Sans CJK SC", font_size=26, color=self.COLOR_BORROW),
            Text("十位相减时要多减1", font="Noto Sans CJK SC", font_size=26, color=self.COLOR_TEXT)
