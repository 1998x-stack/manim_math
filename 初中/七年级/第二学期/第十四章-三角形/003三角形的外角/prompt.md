...
<problem>
证明三角形的外角是等于不相邻的两个内角之和,过那个外角做一个辅助线平行另外一条边,然后分别用内错角和同位角进行一个证明,在证明的过程中,在manim使用过程中,尤其是要知道这个角它的一个方向性,角不不仅仅是有两个边,关键是起始边和终止边,这个一定要明确的,千万不能搞错这个边,哪个是起始边,哪个是终止边,对应的角。
</problem>
