...
内容: 三种平行线判定方法（同位角相等、内错角相等、同旁内角互补）
目标观众: 七年级学生
...
class ParallelLineDetermination(Scene):
    ...
    def show_corresponding_angles(self):
        ...
        # 顶点在intersection1，一边朝右(平行线1方向)，一边朝右下(截线方向)
        # 创建用于角度测量的线段
        line1_parallel = Line(self.intersection1, self.point_on_l1_right, color=self.COLOR_PRIMARY)
        line1_transversal = Line(self.intersection1, self.point_on_t_below_i1, color=self.COLOR_SECONDARY)
        angle1 = Angle(line1_parallel, line1_transversal,
                      radius=0.5,
                      quadrant=(1, -1),  # 调整象限使角度弧出现在合适位置
                      color=self.COLOR_HIGHLIGHT)
        
        # 创建第二个同位角：截线与平行线2在右上方形成的角
        line2_parallel = Line(self.intersection2, self.point_on_l2_right, color=self.COLOR_PRIMARY)
        line2_transversal = Line(self.intersection2, self.point_on_t_below_i2, color=self.COLOR_SECONDARY)
        angle2 = Angle(line2_parallel, line2_transversal,
                      radius=0.5,
                      quadrant=(1, -1),  # 调整象限使角度弧出现在合适位置
                      color=self.COLOR_HIGHLIGHT)
    ...
    def show_alternate_angles(self):
        ...
        alt_line1_parallel = Line(self.intersection1, self.point_on_l1_left, color=self.COLOR_PRIMARY)
        alt_line1_transversal = Line(self.intersection1, self.point_on_t_below_i1, color=self.COLOR_SECONDARY)
        alternate_angle1 = Angle(alt_line1_parallel, alt_line1_transversal,
                                radius=0.5,
                                quadrant=(-1, -1),  # 调整象限使角度弧出现在内部
                                color=self.COLOR_HIGHLIGHT)
        
        # 创建第二个内错角：截线与平行线2在右上方形成的角
        alt_line2_parallel = Line(self.intersection2, self.point_on_l2_right, color=self.COLOR_PRIMARY)
        alt_line2_transversal = Line(self.intersection2, self.point_on_t_above_i2, color=self.COLOR_SECONDARY)
        alternate_angle2 = Angle(alt_line2_parallel, alt_line2_transversal,
                                radius=0.5,
                                quadrant=(1, 1),  # 调整象限使角度弧出现在内部
                                color=self.COLOR_HIGHLIGHT)
    ...
    def show_co_interior_angles(self):
        ...
        co_int_line1_parallel = Line(self.intersection1, self.point_on_l1_right, color=self.COLOR_PRIMARY)
        co_int_line1_transversal = Line(self.intersection1, self.point_on_t_below_i1, color=self.COLOR_SECONDARY)
        co_interior_angle1 = Angle(co_int_line1_parallel, co_int_line1_transversal,
                                  radius=0.6,
                                  quadrant=(1, -1),  # 调整象限使角度弧出现在内部
                                  color=self.COLOR_HIGHLIGHT)
        
        # 创建同旁内角2：截线与平行线2在右上方形成的角（与上面的角在同一侧）
        co_int_line2_parallel = Line(self.intersection2, self.point_on_l2_right, color=self.COLOR_PRIMARY)
        co_int_line2_transversal = Line(self.intersection2, self.point_on_t_above_i2, color=self.COLOR_SECONDARY)
        co_interior_angle2 = Angle(co_int_line2_parallel, co_int_line2_transversal,
                                  radius=0.6,
                                  quadrant=(1, 1),  # 调整象限使角度弧出现在内部
                                  color=self.COLOR_HIGHLIGHT)
...
if __name__ == "__main__":
    # This section allows running the verification independently
    # But the main functionality is the animation scene
    scene = ParallelLineDetermination()
    print("Animation code prepared. To render, run:")
