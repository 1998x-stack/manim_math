...
保留原有 AlgebraicExpressionConcept 场景入口及七段教学流程。
...
class AlgebraicExpressionConcept(Scene):
    ...
    def show_opening(self):
        ...
        examples = VGroup(
            MathTex("3+5", color=WHITE), MathTex("a+b", color=BLUE_C),
            MathTex("3x^2y", color=RED_C), MathTex("2x^2+3x-1", color=GREEN_C),
        ).arrange(DOWN, buff=0.65).move_to(UP * 1.2)
        question = self.caption("它们之间有什么关系？", DOWN * 3.7, YELLOW)
        self.play(Write(title), run_time=0.7)
        for expression in examples:
            self.play(FadeIn(expression, shift=UP * 0.2), run_time=0.4)
    ...
    def show_algebraic_expression(self):
        ...
        definition = self.caption("数、字母及其通过运算组成的式子", UP * 3.6)
        concrete = MathTex("3", "+", "5", font_size=56).move_to(UP * 1.5)
        abstract = MathTex("a", "+", "b", font_size=56, color=BLUE_C)
        abstract.move_to(DOWN * 0.3)
        plus_box = SurroundingRectangle(abstract[1], color=YELLOW, buff=0.12)
        label = self.caption("运算符号：加、减、乘、除、乘方等", DOWN * 2.0)
        examples = self.fit(MathTex(
            r"2x-y\qquad x^2+1\qquad \frac{1}{x}", font_size=34
        )).move_to(DOWN * 4.5)
        ...
        self.play(Create(plus_box), FadeIn(label), run_time=0.6)
    ...
    def show_monomial_definition(self):
        ...
        definition = self.caption("数与字母的乘积；单独的数或字母也算", UP * 3.7)
        ...
        examples = self.fit(MathTex(r"5\qquad x\qquad -2ab", font_size=38))
        examples.move_to(DOWN * 4.1)
        ...
        self.play(FadeIn(examples), FadeIn(non_example), run_time=0.6)
        self.wait(1)
        self.clear_content()
    ...
    def show_polynomial(self):
        ...
        boxes = VGroup(*[
            SurroundingRectangle(term, color=ORANGE_C, buff=0.13) for term in terms
        ])
        labels = VGroup(*[
            Text("单项式", font_size=21, color=GRAY_A).next_to(term, DOWN, buff=0.35)
            for term in terms
        ])
    ...
    def show_wholestyle_summary(self):
        ...
        # 卡片中心相距 3.8，宽度均为 3.5，因此没有重叠。
        card_data = (
            ("单项式", r"3x^2y", RED_C, -1.9),
            ("多项式", r"2x^2+3x-1", GREEN_C, 1.9),
        )
        cards = VGroup()
        for name, expression, color, x_pos in card_data:
            ...
            formula = self.fit(MathTex(expression, font_size=36, color=color), width=3.1)
            formula.move_to(box.get_center() + DOWN * 0.45)
            cards.add(VGroup(box, label, formula).move_to([x_pos, 0.9, 0]))
