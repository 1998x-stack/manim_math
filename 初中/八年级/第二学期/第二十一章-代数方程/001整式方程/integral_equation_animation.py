...
class IntegralEquation(Scene):
    """沿用原七镜和公开 Scene 类名，按因式分解的数学链条组织动画。"""
    ...
    def cn(self, text, y=0, size=29, color=WHITE):
        result = Text(text, font_size=size, color=color).move_to([0, y, 0])
        if result.width > 7.6:
    ...
    def scene1_hook(self):
        ...
        self.play(Write(title), Write(equation), run_time=1.0)
    ...
    def scene2_definition(self):
        ...
        definition = self.cn("分母中不含未知数，整理后可写成整式等式", 4.3, 26, WHITE)
        examples = VGroup(
        ...
        notes = self.cn("本课重点：用因式分解求三次整式方程的实数解", -3.2,
                        26, RESULT_COLOR)
    ...
    def scene4_example(self):
        ...
        ).arrange(DOWN, buff=0.52).move_to(UP * 2.5)
        first_note = self.cn("提取公因式 x；平方差分解", -0.5, 27, GRAY_A)
        roots_line = self.tex(r"x=0\quad\text{或}\quad x=-2\quad\text{或}\quad x=2",
                              -1.6, 32, RESULT_COLOR)
        # 独立数轴示意：画面上三个点与数学模型算出的三个根一一对应。
        number_line = NumberLine(x_range=[-3, 3, 1], length=6.0,
                                 include_numbers=True, include_tip=False,
                                 color=GRAY_B).move_to(DOWN * 3.5)
        dots = VGroup(*[Dot(number_line.n2p(root), radius=0.095, color=RESULT_COLOR)
                        for root in self.roots])
        tick_labels = VGroup(*[self.tex(str(int(root)), size=27, color=RESULT_COLOR)
                               .next_to(number_line.n2p(root), DOWN, buff=0.3)
                               for root in self.roots])
        self.play(Write(title), run_time=0.5)
        for formula in formulae:
            self.play(Write(formula), run_time=0.55)
        self.play(FadeIn(first_note), Write(roots_line), run_time=0.7)
        self.play(Create(number_line), FadeIn(dots), FadeIn(tick_labels), run_time=0.9)
        self.wait(0.9)
        self.play(FadeOut(title), FadeOut(formulae), FadeOut(first_note),
                  FadeOut(roots_line), FadeOut(number_line), FadeOut(dots),
                  FadeOut(tick_labels), run_time=0.5)

    def scene5_zero_product(self):
        title = self.cn("零乘积法则", 5.6, 39, GOLD)
        equation = self.tex(r"A\cdot B\cdot C=0", 3.0, 52, MAIN_COLOR)
        ...
        self.play(Write(title), Write(equation), run_time=0.8)
        self.play(FadeIn(premise), FadeIn(branches), run_time=0.7)
        self.play(FadeIn(note), FadeIn(lost_root), run_time=0.7)
        self.wait(0.9)
        self.play(FadeOut(title), FadeOut(equation), FadeOut(premise),
    ...
    def scene6_cube_type(self):
        ...
        self.wait(0.9)
        self.play(FadeOut(title), FadeOut(example), FadeOut(step),
                  FadeOut(verify), FadeOut(general), FadeOut(restriction),
                  FadeOut(contrast), run_time=0.5)

    def scene7_outro(self):
        heading = self.cn("整式方程：方法与检查", 5.5, 37, GOLD)
        card_text = VGroup(
            self.cn("第一步：整理并因式分解", size=29, color=STEP_COLOR),
            self.cn("第二步：各因式分别等于零", size=29, color=STEP_COLOR),
            self.cn("第三步：逐一代回原方程", size=29, color=RESULT_COLOR),
        ).arrange(DOWN, buff=0.7).move_to(UP * 1.3)
        verification = self.tex(r"p(-2)=p(0)=p(2)=0", -2.1, 35, MAIN_COLOR)
        ending = self.cn("@emptyandcalm", -4.3, 27, GRAY_B)
        self.play(Write(heading), run_time=0.5)
        for row in card_text:
            self.play(FadeIn(row), run_time=0.4)
        self.play(Write(verification), FadeIn(ending), run_time=0.6)
        self.wait(0.9)
        self.play(FadeOut(heading), FadeOut(card_text), FadeOut(verification),
