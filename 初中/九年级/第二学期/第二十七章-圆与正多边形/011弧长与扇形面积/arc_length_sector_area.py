...
class ArcLengthAndSectorArea(Scene):
    ...
    def construct(self):
        ...
        self.sector = Sector(radius=self.radius, start_angle=0, angle=self.theta,
                             arc_center=self.center, color=self.C_ARC, fill_opacity=0.26,
                             stroke_width=2)
        self.center_dot = Dot(self.center, radius=0.09, color=self.C_HIGHLIGHT)
        self.radii = VGroup(Line(self.center, self.center + self.radius * RIGHT,
    ...
    def heading(self, words):
        return Text(words, font="sans-serif", font_size=35, color=self.C_HIGHLIGHT).move_to(UP * 5.3)

    def arc_label(self, words, radius_offset=0.48):
        angle = self.theta / 2
        position = self.center + (self.radius + radius_offset) *
    ...
    def scene_2_central_angle(self):
        ...
        measured = MathTex(r"\theta=60^\circ", font_size=35, color=self.C_HIGHLIGHT).move_to(DOWN * 4.2)
    ...
    def scene_3_arc_length_formula(self):
        ...
        proportion = MathTex(r"\frac{l}{2\pi R}=\frac{n}{360}", font_size=36).move_to(DOWN * 4.1)
    ...
    def scene_4_arc_length_example(self):
        ...
        decimal = Text(f"弧长约为 {self.data['arc_length']:.2f}（长度单位）",
                       font="sans-serif", font_size=26, color=GRAY_A).move_to(DOWN * 5.25)
    ...
    def scene_5_sector_definition(self):
        ...
        radial = MathTex(r"R=2,\quad\theta=\frac{\pi}{3}", font_size=35).move_to(DOWN * 4.25)
        note = Text("红色填充区域才是扇形；红色曲线是对应的弧", font="sans-serif",
    ...
    def scene_8_comprehensive_example(self):
        ...
        arc_caption = MathTex(r"l=2\pi", font_size=31,
                              color=self.C_ARC).move_to(center + 3.55 *
                              np.array([np.cos(PI / 3), np.sin(PI / 3), 0.0]))
