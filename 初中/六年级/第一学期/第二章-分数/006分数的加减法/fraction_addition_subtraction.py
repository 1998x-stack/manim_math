...
class FractionAdditionSubtraction(Scene):
    ...
    def construct(self):
        ...
        self.COLOR_DENOMINATOR = "#9b59b6"    # 紫色 - 分母
        self.COLOR_NUMERATOR = "#f39c12"      # 橙色 - 分子
    ...
    def setup_geometry(self):
        ...
        # 扇形角度
        self.angle_quarter = PI / 2    # 1/4
        self.angle_half = PI            # 1/2
        self.angle_third = 2 * PI / 3   # 1/3
        self.angle_sixth = PI / 3       # 1/6
        self.angle_two_thirds = 4 * PI / 3  # 2/3
    ...
    def scene_2_same_denominator_addition(self):
        """场景2: 同分母加法 - 1/4 + 1/4 = 2/4 = 1/2 (5-18秒)"""
        # 标题
        title = Text(
            "同分母分数相加",
            font="Noto Sans CJK SC",
        ...
        circle_1 = Circle(
            ...
            color=WHITE,
            stroke_width=3
        ).move_to(self.circle_left_pos)
        
        circle_2 = Circle(
            radius=self.circle_radius,
            color=WHITE,
            stroke_width=3
        ).move_to(self.circle_right_pos)
        ...
        result_circle = Circle(
            ...
            color=WHITE,
            stroke_width=3
        ).move_to(self.circle_result_pos)
        
        result_sector = Sector(
            radius=self.circle_radius,
            angle=self.angle_half,  # 2/4 = 180度
            start_angle=0,
        ...
        self.play(
            FadeOut(result_2_4),
            TransformFromCopy(result_2_4, result_2_4_copy),
            run_time=0.5
        )
        ...
        self.play(
            ...
            FadeOut(result_sector),
            FadeOut(simplify_text),
            FadeOut(result_2_4_copy),
            FadeOut(arrow),
            FadeOut(result_1_2_final),
            run_time=0.6
        )
    
    def scene_3_same_denominator_subtraction(self):
        """场景3: 同分母减法 - 3/5 - 1/5 = 2/5 (18-28秒)"""
        # 标题
        title = Text(
            "同分母分数相减",
            font="Noto Sans CJK SC",
        ...
        # 创建5个矩形块
        blocks = VGroup()
        for i, x_pos in enumerate(self.block_positions_5):
            block = Rectangle(
                width=self.block_width,
                height=self.block_height,
                color=WHITE,
                stroke_width=3
            ).move_to(np.array([x_pos, self.rect_y, 0]))
            blocks.add(block)
        ...
        for i in range(3):
            filled = blocks[i].copy().set_fill(self.COLOR_PRIMARY, opacity=0.7)
            filled_blocks.add(filled)
        ...
        self.play(
            Transform(label_3_5, operation[0]),
            FadeIn(minus_sign),
            Write(label_1_5),
            run_time=0.5
        )
        ...
        # 一个块褪色 (第3个块，索引2)
        self.play(
            filled_blocks[2].animate.set_fill(opacity=0.2),
            FadeOut(explain),
            run_time=0.8
        )
    ...
    def scene_4_different_denominator_problem(self):
        ...
        self.play(
            FadeOut(circle_left),
            FadeOut(circle_right),
            FadeOut(sector_half),
            FadeOut(sector_third),
    ...
    def scene_5_common_denominator_process(self):
        ...
        # 初始：2等分线
        division_2 = Line(
            circle_left.get_center() + UP * 1.0,
            circle_left.get_center() + DOWN * 1.0,
            color=GRAY_B,
            stroke_width=2
        )
        
        # 1/2扇形
        sector_2_initial = Sector(
            radius=1.0,
            angle=PI,
            start_angle=0,
            color=self.COLOR_PRIMARY,
            fill_opacity=0.6,
            stroke_width=0
        ).move_to(circle_left.get_center())
        
        self.play(
            Create(circle_left),
            Create(division_2),
            FadeIn(sector_2_initial),
            run_time=0.8
        )
        ...
        for i in range(6):
            angle = i * PI / 3
            end_point = circle_left.get_center() + np.array([
                1.0 * np.cos(angle),
                1.0 * np.sin(angle),
                0
            ])
        ...
        transform_left = MathTex(
            ...
            font_size=28,
            color=WHITE
        ).next_to(arrow_left, RIGHT, buff=0.2)
        
        self.play(
            GrowArrow(arrow_left),
            Write(transform_left),
            run_time=0.8
        )
        ...
        for i in range(3):
            angle = i * 2 * PI / 3
            end_point = circle_right.get_center() + np.array([
                1.0 * np.cos(angle),
                1.0 * np.sin(angle),
                0
            ])
            ...
            divisions_3.add(line)
        
        # 1/3扇形
        sector_3_initial = Sector(
            radius=1.0,
            angle=2 * PI / 3,
            start_angle=0,
            color=self.COLOR_SECONDARY,
            fill_opacity=0.6,
            stroke_width=0
        ).move_to(circle_right.get_center())
        
        self.play(
            Create(circle_right),
            Create(divisions_3),
            FadeIn(sector_3_initial),
            run_time=0.8
        )
        ...
        transform_right = MathTex(
            ...
            font_size=28,
            color=WHITE
        ).next_to(arrow_right, LEFT, buff=0.2)
        
        self.play(
            GrowArrow(arrow_right),
            Write(transform_right),
            run_time=0.8
        )
        ...
        for i in range(5):
            sector = Sector(
                ...
                fill_opacity=0.7,
                stroke_width=0
            ).move_to(result_circle.get_center())
            sectors_result.add(sector)
        
        # 分割线
        divisions_result = VGroup()
        for i in range(6):
            angle = i * PI / 3
            end_point = result_circle.get_center() + np.array([
                1.0 * np.cos(angle),
                1.0 * np.sin(angle),
                0
            ])
            line = Line(
                result_circle.get_center(),
                end_point,
                color=GRAY_B,
                stroke_width=2
            )
            divisions_result.add(line)
        
        self.play(
            Create(result_circle),
            Create(divisions_result),
            FadeIn(sectors_result),
            run_time=1.0
        )
        ...
        result_text = MathTex(
            ...
            font_size=40,
            color=self.COLOR_RESULT
        ).next_to(result_circle, DOWN, buff=0.5)
        ...
        self.play(
            ...
            FadeOut(circle_right),
            FadeOut(division_2),
            FadeOut(sector_2_initial),
            FadeOut(divisions_3),
            FadeOut(sector_3_initial),
            FadeOut(arrow_left),
            FadeOut(arrow_right),
            FadeOut(transform_left),
            FadeOut(transform_right),
            FadeOut(step2_title),
            FadeOut(addition_formula),
            FadeOut(result_circle),
            FadeOut(divisions_result),
            FadeOut(sectors_result),
            FadeOut(result_text),
            run_time=0.6
        )
    
    def scene_6_different_denominator_subtraction(self):
        """场景6: 异分母减法示例 - 2/3 - 1/2 (48-58秒)"""
        # 题目
        problem = MathTex(
            r"\frac{2}{3} - \frac{1}{2}",
            font_size=40,
        ...
        # 矩形块演示（6块）
        blocks = VGroup()
        for i, x_pos in enumerate(self.block_positions_6):
            block = Rectangle(
                width=self.block_width * 0.85,
                height=self.block_height,
                color=WHITE,
                stroke_width=2
            ).move_to(np.array([x_pos, 1.5, 0]))
            blocks.add(block)
    ...
    def scene_7_summary_outro(self):
        ...
        for i, pos in enumerate(positions):
            frac = self.create_fraction_display(
                (i % 3) + 1,
                (i % 4) + 2,
                [self.COLOR_PRIMARY, self.COLOR_SECONDARY, 
                 self.COLOR_RESULT, self.COLOR_HIGHLIGHT][i],
                24
            ).move_to(pos).set_opacity(0.6)
            fractions_deco.add(frac)
        
        self.play(
            *[FadeIn(frac, scale=0.5) for frac in fractions_deco],
            run_time=0.6
        )
        
        self.play(
            Rotate(fractions_deco, angle=PI/4, run_time=1.5)
        )
    ...
    def create_summary_card(self, title, content, formula, color, position):
        ...
        # 组合
        text_group = VGroup(title_text, content_text, formula_text).arrange(DOWN, buff=0.15, aligned_edge=LEFT)
