...
class TongFenAnimation(Scene):
    ...
    def show_visual_problem(self):
        """场景2: 问题可视化 - 饼图展示"""
        # 移动分数到饼图上方
        frac_1_top = self.frac_1_original.copy()
        frac_2_top = self.frac_2_original.copy()
        ...
        # 分成3份的分割线
        pie1_divisions = VGroup()
        for i in range(3):
            angle = i * 2 * PI / 3
            line = Line(
                self.pie1_center,
                self.pie1_center + self.pie_radius * np.array([np.cos(angle), np.sin(angle), 0]),
                color=self.COLOR_PRIMARY,
                stroke_width=2
            )
            pie1_divisions.add(line)
        
        self.play(Create(pie1_divisions), run_time=0.5)
        
        # 填充1份 (1/3)
        angle_1_3 = 2 * PI / 3
        pie1_fill = AnnularSector(
            inner_radius=0,
        ...
        # 分成4份的分割线
        pie2_divisions = VGroup()
        for i in range(4):
            angle = i * PI / 2
            line = Line(
                self.pie2_center,
                self.pie2_center + self.pie_radius * np.array([np.cos(angle), np.sin(angle), 0]),
                color=self.COLOR_SECONDARY,
                stroke_width=2
            )
            pie2_divisions.add(line)
        
        self.play(Create(pie2_divisions), run_time=0.5)
        
        # 填充1份 (1/4)
        angle_1_4 = PI / 2
        pie2_fill = AnnularSector(
            inner_radius=0,
            outer_radius=self.pie_radius,
            angle=angle_1_4,
            start_angle=PI / 2,
    ...
    def show_lcm_finding(self):
        ...
        # 标记12
        twelve_3 = nums_3[3]  # 12在3的倍数中
        twelve_4 = nums_4[2]  # 12在4的倍数中
    ...
    def show_convert_1_3(self):
        ...
        for i in range(12):
            ...
            pie1_12_divisions.add(line)
        
        pie1_12_fill = AnnularSector(
            inner_radius=0,
            outer_radius=self.pie_radius,
            angle=4 * 2 * PI / 12,  # 4份
            start_angle=PI / 2,
    ...
    def show_convert_1_4(self):
        ...
        for i in range(12):
            ...
            pie2_12_divisions.add(line)
        
        pie2_12_fill = AnnularSector(
            inner_radius=0,
            outer_radius=self.pie_radius,
            angle=3 * 2 * PI / 12,  # 3份
            start_angle=PI / 2,
    ...
    def show_summary(self):
        ...
        # 合并饼图显示7/12
        combined_pie_fill = AnnularSector(
            inner_radius=0,
            outer_radius=self.pie_radius,
            angle=7 * 2 * PI / 12,  # 7份
            start_angle=PI / 2,
        ...
        combined_pie_divisions = VGroup()
        for i in range(12):
            angle = i * 2 * PI / 12
            line = Line(
                ORIGIN,
                ORIGIN + self.pie_radius * np.array([np.cos(angle), np.sin(angle), 0]),
                color=self.COLOR_COMMON,
                stroke_width=2
            )
            combined_pie_divisions.add(line)
    ...
    def show_outro(self):
        ...
        self.play(
            *[FadeIn(icon, scale=0.5) for icon in fraction_icons],
            run_time=0.6
        )
