...
class SpatialLinesScene(Scene):
    ...
    def scene_5_skew_angle(self):
        ...
        theta_label = MathTex(r"\theta", color=C_ANGLE, font_size=30).move_to(
            O + 0.75 * (v_a[:2]/np.linalg.norm(v_a[:2]) + v_b[:2]/np.linalg.norm(v_b[:2]))/2 * np.array([1,1,0]) + np.array([0, 0, 0])
        )

        # 保险起见直接用fixed position
        ...
        theta_pos = O + np.array([mid_dir_2d[0], mid_dir_2d[1], 0]) * 0.8
