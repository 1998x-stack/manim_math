...
2. **构建分镜脚本** - 给予<problem>涉及的题目/知识点内容，创建 `storyboard.md`，包含详细场景、几何计算、元素生命周期管理
...
4. **verify_geometry.py本地运行获取反馈** - verify_geometry.py本地运行获取反馈，然后fix&repair python code
    ...
    - build def grep_MathTex() function: avoid for LaTeX compilation error(such as LaTeX Error: Unicode character 乘 (U+4E58) ); **核心原则**：`MathTex` 只能接受纯 LaTeX 命令，**所有中文（包括 `_{}` 下标、`\text{}` 内）必须用 `Text(font="Noto Sans CJK SC")` 单独渲染**，再用 `VGroup(...).arrange(RIGHT)` 拼合。
...
<problem>
{
  ...
  "内容": "三角函数",
  "知识点": "正弦函数和余弦函数的图像与性质",
  "知识点内容详细描述": "正弦函数y=sin x：定义域R，值域[-1,1]，周期2π，奇函数，在[-π/2,π/2]上单调递增。图像是正弦曲线，关于原点对称。余弦函数y=cos x：定义域R，值域[-1,1]，周期2π，偶函数，在[0,π]上单调递减。图像关于y轴对称。关系：cos x=sin(x+π/2)，sin x=cos(π/2-x)。「五点法」画图：取0, π/2, π, 3π/2, 2π五个关键点。",
  "数学公式": [
    "y = sin x：定义域R，值域[-1,1]，周期2π",
    "y = cos x：定义域R，值域[-1,1]，周期2π",
    "sin x最大值1，x = π/2 + 2kπ",
    "sin x最小值-1，x = -π/2 + 2kπ",
    "cos x = sin(x + π/2)",
    "sin x = cos(π/2 - x)"
  ],
  "相关知识点": [
