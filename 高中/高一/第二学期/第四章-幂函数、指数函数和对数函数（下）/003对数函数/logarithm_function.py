"""高一第二学期《对数函数》；保留七段原教学结构与 LogarithmFunction 入口。

严格区分数学定义域 (0,+∞) 与屏上可见的有限绘图区间 [1/8,6]。
执行：manim -ql logarithm_function.py LogarithmFunction
实际字体/TeX、关键帧、视频与音轨验收另行记录。
"""
from manim import *
import math
...
def cn(text, font_size=27, color=WHITE):
    return Text(text, font=FONT, font_size=font_size, color=color)


def fit(item, max_width=7.5):
...
class LogarithmFunction(Scene):
    ...
    def _title(self, text):
        return fit(cn(text, 37, YELLOW)).move_to(UP * 6.4)

    def _clear(self, *objects):
        self.play(*[FadeOut(item) for item in objects], run_time=0.5)
    ...
    def show_opening(self):
        ...
        note = cn("观察图像与底数的关系", 27, GRAY_A).move_to(DOWN * 3.0)
    ...
    def show_definition(self):
        ...
        conditions = MathTex(r"a>0,\quad a\ne 1,\quad x>0", font_size=35).move_to(UP * 2.0)
    ...
    def show_axes(self):
        ...
        self.axes = Axes(x_range=[0, 6, 1], y_range=[-3, 3, 1],
        ...
        note = cn("x=0 是垂直渐近线，不属于函数定义域", 24, ASYMPTOTE_COLOR)
        fit(note).move_to(DOWN * 4.5)
    ...
    def show_case_a_greater_than_1(self):
        ...
        # log₂(1/8)=-3，log₂6<3，整段实际曲线处于 y∈[-3,3]。
        self.graph_increase = self.axes.plot(lambda x: math.log2(x),
                                             x_range=[0.125, 6], color=UP_COLOR,
                                             stroke_width=4)
        points = VGroup()
        for x, y in [(1, 0), (2, 1), (4, 2)]:
            dot = Dot(self.axes.c2p(x, y), radius=0.065, color=POINT_COLOR)
            label = MathTex(r"(1,0)" if x == 1 else (r"(2,1)" if x == 2 else r"(4,2)"),
                            font_size=24, color=WHITE).next_to(dot, UR, buff=0.12)
    ...
    def show_case_a_less_than_1(self):
        ...
        # log_(1/2)(1/8)=3，log_(1/2)6>-3，屏上两曲线处于同一轴域。
        ...
        for x, y in [(1, 0), (2, -1), (4, -2)]:
            dot = Dot(self.axes.c2p(x, y), radius=0.065, color=POINT_COLOR)
            label = MathTex(r"(1,0)" if x == 1 else (r"(2,-1)" if x == 2 else r"(4,-2)"),
                            font_size=24, color=WHITE).next_to(dot, DR, buff=0.12)
            points.add(dot, label)
    ...
    def show_summary(self):
        ...
        lines = VGroup(
            MathTex(r"a>0,\ a\ne1,\ x>0", font_size=35, color=WHITE),
    ...
    def show_outro(self):
        ...
        up_formula = MathTex(r"a>1\ \Rightarrow\ \log_a x\ \text{递增}", font_size=32,
                             color=UP_COLOR).move_to(UP * 2.8)
        down_formula = MathTex(r"0<a<1\ \Rightarrow\ \log_a x\ \text{递减}", font_size=32,
                               color=DOWN_COLOR).move_to(UP * 0.4)
        # 标准 LaTeX 的 \text{...} 不含中文；中文说明另用 Text 绘制。
        up_formula = MathTex(r"a>1\ \Rightarrow\ f\text{ increasing}", font_size=32,
                             color=UP_COLOR).move_to(UP * 2.8)
        down_formula = MathTex(r"0<a<1\ \Rightarrow\ f\text{ decreasing}", font_size=32,
                               color=DOWN_COLOR).move_to(UP * 0.4)
        recap = cn("两种情形都经过 (1,0)", 31, POINT_COLOR).move_to(DOWN * 2)
        finish = cn("掌握图像，理解对数函数", 28, YELLOW).move_to(DOWN * 4)
        self.play(Write(title), Write(up_formula), run_time=0.8)
        self.play(Write(down_formula), FadeIn(recap), run_time=0.8)
        self.play(FadeIn(finish), run_time=0.7)
        self.wait(1.2)
        self._clear(title, up_formula, down_formula, recap, finish, self.author_info)
