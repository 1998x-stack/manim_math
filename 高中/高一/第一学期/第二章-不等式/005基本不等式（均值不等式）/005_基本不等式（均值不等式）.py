...
class BasicInequalityAnimation(Scene):
    ...
    def show_geometric_interpretation(self):
        ...
        rect_label = MathTex(f"\\text{{矩形: 长={self.a}, 宽={self.b}, 靣积={self.a*self.b}}}", color=WHITE).next_to(rectangle, UP, buff=0.2)
        ...
        square_label = MathTex(f"\\text{{正方形: 边长=\\sqrt{{ab}}={square_side:.3f}, 靣积={self.a*self.b}}}", color=GREEN).next_to(square, DOWN, buff=0.2)
        ...
        perimeter_square_label = MathTex(
            f"\\text{{周长相同正方形: 边长=\\frac{{a+b}}{{2}}={perimeter_square_side}, 周长={2*(self.a+self.b)}}}", 
            color=RED
        ).next_to(perimeter_square, LEFT, buff=0.2)
    ...
    def show_algebraic_proof(self):
        ...
        # 显示等号成立条件
        condition = MathTex("\\text{等号成立当且仅当 } a = b", font_size=28, color=self.COLOR_HIGHLIGHT).move_to(DOWN * 6)
    ...
    def show_means_chain_inequality(self):
        ...
        means_explanation = VGroup(
            MathTex("H = \\text{调和平均数}", color=PURPLE).move_to(UP * 3),
            MathTex("G = \\text{几何平均数}", color=GREEN).move_to(UP * 2),
            MathTex("A = \\text{算术平均数}", color=BLUE).move_to(UP * 1),
            MathTex("Q = \\text{平方平均数}", color=ORANGE).move_to(ORIGIN)
        )
